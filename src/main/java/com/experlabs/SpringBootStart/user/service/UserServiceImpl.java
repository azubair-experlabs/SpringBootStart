...
public class UserServiceImpl implements UserService {
  ...
  public User getUserByID(Long id) {
    ...
    return response.orElseThrow(
        () -> new EntityNotFoundException(String.format("user not found with id %d", id)));
  }
  ...
  public User updateUserByID(Long id, String name, String email, String password) {
    ...
    if (!StringUtil.isNullOrEmpty(password) && !passwordEncoder.matches(password,
        user.getPassword())) {
      user.setPassword(passwordEncoder.encode(password));
    }
    if (!StringUtil.isNullOrEmpty(email) && !email.equals(user.getName())
